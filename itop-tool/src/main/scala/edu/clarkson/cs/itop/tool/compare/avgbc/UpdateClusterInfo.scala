 ...
 */
class AddToMapper extends SingleKeyJoinMapper("merge_decision", "cluster_info_nofrom", 1, 0);
...
class AddToReducer extends RightOuterJoinReducer(
  ...
  (key, left, right) => {
    if (left == null) {
      ...
      var oldSize = right(1).toString.toInt;
      var addedSize = left(2).toString.toInt;
      var newSize = oldSize + addedSize;
      (new Text(right(0).toString), new Text(Array(newSize.toString, right(2).toString).mkString("\t")));
    }
  });