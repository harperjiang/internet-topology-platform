 ...
 */
class InitClusterMapper extends Mapper[Object, Text, IntWritable, StringArrayWritable] {
  ...
  override def map(key: Object, value: Text, context: Mapper[Object, Text, IntWritable, StringArrayWritable]#Context) = {
    ...
    fileName match {
      case "node_link" => {
        ...
        context.write(new IntWritable(values(1).toInt), new StringArrayWritable(Array("node_link", values(0))));
      }
      case "node_degree" => {
        var values = value.toString().split("\\s+");
        context.write(new IntWritable(values(0).toInt), new StringArrayWritable(Array("node_degree", values(1))));
      }
...
class InitClusterReducer extends Reducer[IntWritable, StringArrayWritable, IntWritable, Text] {
    ...
    values.foreach { value =>
      {
        value.get()(0).toString() match {
          ...
          case "node_link" => { links += value.get()(1).toString().toInt };
          case _ => { throw new IllegalArgumentException(value.get()(0).toString()) };
        }
      }
    };
    links.foreach(link => { context.write(key, new Text(Array(link, degree).mkString("\t"))) });
  }
}
